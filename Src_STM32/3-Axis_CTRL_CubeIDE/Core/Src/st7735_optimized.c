...
// Hàm in chuỗi ký tự siêu tốc bằng DMA - Tận dụng bộ quét dòng PushLine của bác
void ST7735_DrawString_DMA(uint16_t x, uint16_t y, const char *str, FontDef font, uint16_t fg, uint16_t bg) {
    // Nếu dòng chữ nằm ngoài màn hình thì nghỉ khỏe
    if (y + font.height > ST7735_HEIGHT) return;
    
    ST7735_Select();
    
    // --- BƯỚC 1: GIỚI HẠN VÙNG VẼ (WINDOW) QUÉT NGANG TOÀN MÀN HÌNH ---
    // Thiết lập chiều rộng từ cột 0 đến cột 159 (Bao trọn dòng)
    ST7735_WriteCmd(ST7735_CASET);
    uint8_t caset[] = {0x00, 0x00, 0x00, ST7735_WIDTH - 1};
    ST7735_WriteData(caset, 4);
    
    // Thiết lập chiều cao từ hàng y đến hết chiều cao font chữ
    ST7735_WriteCmd(ST7735_RASET);
    uint8_t raset[] = {0x00, y, 0x00, y + font.height - 1};
    ST7735_WriteData(raset, 4);
    
    ST7735_WriteCmd(ST7735_RAMWR);
    
    // Tạo một mảng đệm cục bộ chứa dữ liệu của 1 dòng quét (160 pixel)
    uint16_t temp_row_buf[ST7735_WIDTH];
    
    // --- BƯỚC 2: QUÉT THEO TỪNG DÒNG NGANG CỦA FONT (RASTER SCAN) ---
    for (uint16_t py = 0; py < font.height; py++) {
        
        // 1. Việc đầu tiên là nhuộm nền toàn bộ dòng bằng màu Background (Xóa chữ cũ luôn!)
        for (uint16_t i = 0; i < ST7735_WIDTH; i++) {
            temp_row_buf[i] = bg;
        }
        
        // 2. Gom các pixel hàng thứ `py` của TẤT CẢ các ký tự trong chuỗi str vào mảng
        uint16_t cur_x = x;
        const char *ptr = str;
        
        while (*ptr) {
            char c = *ptr;
            if (c >= 32 && c <= 126) { // Bỏ qua ký tự lỗi
                uint16_t char_idx = c - 32;
                // Lấy byte dữ liệu font tại hàng quét `py`
                uint16_t char_data = font.data[char_idx * font.height + py];
                
                // Trích xuất từng bit (pixel) của chữ nạp vào temp_row_buf
                for (uint16_t px = 0; px < font.width; px++) {
                    if (cur_x + px < ST7735_WIDTH) {
                        uint8_t bit = (char_data >> (15 - px)) & 1;
                        if (bit) {
                            temp_row_buf[cur_x + px] = fg; // Điền màu chữ
                        }
                    }
                }
            }
            cur_x += font.width; // Nhảy sang vị trí của chữ tiếp theo
            if (cur_x >= ST7735_WIDTH) break; // Tràn màn hình thì dừng gom bit
            ptr++;
        }
        
        // --- BƯỚC 3: ĐẨY NGUYÊN DÒNG ĐÃ TRỘN CHỮ QUA DMA ---
        // Gọi chính hàm PushLine của bác. Hàm này bốc dữ liệu thả cho DMA chạy ngầm, CPU thoát ra ngay!
        ST7735_PushLine(temp_row_buf);
    }
    
    // Đợi đợt DMA cuối cùng xong xuôi thì nhả chân CS
    ST7735_EndFrame();
}
